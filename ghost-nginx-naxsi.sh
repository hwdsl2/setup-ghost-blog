...
if [ "$ghost_num" = "1" ] || [ ! -f /opt/nginx/sbin/nginx ]; then
  ...
  cd /opt/src || exit 1
  wget -t 3 -T 30 -qO- https://github.com/nbs-system/naxsi/archive/0.55.2.tar.gz | tar xz
  [ ! -d naxsi-0.55.2 ] && { echoerr "Cannot download Naxsi source."; exit 1; }
  
  # Ref: https://github.com/nbs-system/naxsi/commit/c7e8cff427eaf6b6ece1b0b1c8ddaba11bb21d45
  sed -i -e 's/127\.0\.0\.1:443/127.0.0.1:9200/' -e '/use_ssl/s/"True"/false/' naxsi-0.55.2/nxapi/nxapi.json
  ...
  ./configure --add-module=../naxsi-0.55.2/naxsi_src/ \
  ...
  mkdir -p /etc/nginx
  /bin/cp -f /opt/src/naxsi-0.55.2/naxsi_config/naxsi_core.rules /etc/nginx/
...
EOF
  ...
  cd /opt/src/naxsi-0.55.2/nxapi/ && python setup.py install
