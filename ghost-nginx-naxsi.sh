...
page_num=1
until [ -n "$ghost_url" ] || [ "$page_num" -gt 10 ]; do
  ghost_releases="https://api.github.com/repos/TryGhost/Ghost/releases?page=$page_num"
...
if [ "$ghost_num" = "1" ] || [ ! -f /opt/nginx/sbin/nginx ]; then
  ...
  cd /opt/src || exit 1
  wget -t 3 -T 30 -qO- https://nginx.org/download/nginx-1.12.1.tar.gz | tar xz
  [ ! -d nginx-1.12.1 ] && { echoerr "Cannot download Nginx source."; exit 1; }
  cd nginx-1.12.1 || exit 1
